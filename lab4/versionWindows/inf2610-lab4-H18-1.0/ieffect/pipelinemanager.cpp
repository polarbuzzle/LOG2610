...
void PipelineManager::launchParallel(int queueSize)
{
     ...
     */
    HANDLE threads[m_stageList.size()];
    DWORD threadsID[m_stageList.size()];
    for (int i = 0; i < m_stageList.size(); i++) {
        threads[i] = CreateThread(NULL, 0, startHelper, (LPVOID)m_stageList.at(i), 0, &threadsID[i]);
    }
    WaitForMultipleObjects(m_stageList.size(), threads, true, INFINITE);
    for(int i = 0; i < m_stageList.size(); i++) {
